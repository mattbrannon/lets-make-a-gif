...
const setInputOutput = async (req, res, next) => {
  ...
  if (req.baseUrl === '/api/upload/images') {
    // handle images
  ...
  else if (req.baseUrl === '/api/upload/image') {
    // handle single image
  ...
  else if (req.baseUrl === '/api/upload/videos') {
    // handle video
  ...
  else if (req.baseUrl === '/api/upload/filters') {
    // check original path
    if (req.body.route === '/api/upload/images') {
      // handle images
      const pathToOutput = path.resolve(userData.output, `${req.body.filename}.gif`);
      const pathToImages = path.resolve(userData.images, `*${req.body.ext}`);
      userData.pathToInput = pathToImages;
    ...
    else if (req.body.route === '/api/upload/videos') {
      // handle videos
      const file = fs.readdirSync(userData.original)[0];
      const pathToOriginal = path.resolve(userData.original, file);
      const pathToInput = path.resolve(userData.videos, file);
  ...
  else if (req.baseUrl === '/api/upload/reset') {
    // check original path
    if (req.body.route === '/api/upload/images') {
      // handle images
      const pathToOutput = path.resolve(userData.output, `${req.body.filename}.gif`);
